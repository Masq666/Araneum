...
static t_host *new_host(void) {
    ...
    host->trigger_on_cgi_status = true;
...
static t_binding *new_binding(void) {
    ...
    binding->ssl_key_cert         = NULL;
    binding->required_ca          = NULL;
    binding->verify_depth         = 1;
...
t_config *default_config(void) {
    ...
    config->server_string      = PACKAGE_STRING;
    init_groups(&(config->groups));
#ifdef HAVE_SSL
    config->dh_file            = NULL;
    config->allowed_ciphers    = NULL;
#endif
    ...
    config->pidfile            = PIDFILE_DIR"/araneum.pid";
...
int check_configuration(t_config *config) {
    if (config->first_host->hostname.size == 0) {
        fputs("The default website has no hostname.",stderr);
        return -1;
    }

    if (config->first_host->website_root == NULL) {
        fputs("The default website has no websiteroot.",stderr);
        return -1;
    }
    ...
    char c;
    int i;
    size_t len = strlen(config->work_directory);
    ...
    if (config->binding == NULL) {
        fputs("No binding defined.",stderr);
        return -1;
    }
...
#ifdef HAVE_SSL
    if (strcmp(key, "allowedciphers") == 0) {
        if (config->allowed_ciphers == NULL) {
            if ((config->allowed_ciphers = strdup(value)) != NULL) {
                return true;
            }
        } else {
            if ((rest = (char*)malloc(strlen(config->allowed_ciphers) + strlen(value) + 2)) != NULL) {
                sprintf(rest, "%s:%s", config->allowed_ciphers, value);
                free(config->allowed_ciphers);
                config->allowed_ciphers = rest;
                return true;
            }
        }
    } else
...
#ifdef HAVE_SSL
        } else if (strcmp(key, "dhparameters") == 0) {
            if ((config->dh_file = strdup(value)) != NULL) {
                return true;
            }
#endif
        ...
        /* Turn Logging completly off, on by default. */
        } else if (strcmp(key, "logging") == 0) {
            if (parse_yesno(value, &(config->logging)) == 0) {
                return true;
            }
...
static bool user_setting(char *key, char *value, t_host *host, t_tempdata **tempdata) {
    ...
    if (strcmp(key, "accesslist") == 0) {
        ...
        if (parse_keyvaluelist(value, &(host->envir_str), "=") != -1) {
...
static bool host_setting(char *key, char *value, t_host *host) {
    ...
    if (strcmp(key, "accesslogfile") == 0) {
        ...
        if (parse_keyvaluelist(value, &(host->alias), ":") != -1) {
        ...
        if (parse_charlist(strlower(value), &(host->hostname)) == 0) {
            return true;
        }
    ...
    } else if (strcmp(key, "preventcsrf") == 0) {
...
static bool directory_setting(char *key, char *value, t_directory *directory) {
    ...
    if (strcmp(key, "accesslist") == 0) {
        ...
        if (parse_keyvaluelist(value, &(directory->envir_str), "=") != -1) {
            return true;
        }
    } else if (strcmp(key, "startfile") == 0) {
        if (valid_start_file(value)) {
...
static bool binding_setting(char *key, char *value, t_binding *binding) {
    ...
    if (strcmp(key, "enablealter") == 0) {
        ...
        if ((binding->required_ca = strdup(value)) != NULL) {
            if (rest != NULL) {
                if ((binding->verify_depth = str2int(rest)) < 1) {
                    return false;
                }
            }
            return true;
        ...
        if ((binding->ssl_key_cert = strdup(value)) != NULL) {
    ...
    if (strcmp(key, "connectto") == 0) {
        while (value != NULL) {
            ...
            if (*value == '/') {
                ...
                if (parse_ip_port(value, &(connect_to->host), &(connect_to->port)) == -1) {
                    return false;
                }
            }
            value = rest;
...
int read_main_configfile(char *configfile, t_config *config, bool config_check) {
    ...
    /* Read and parse Araneum config file. */
            ...
            if (key[strlen(key) - 1] == '{') {
                ...
                case binding:
                    if (current_binding->port == -1) {
                        ...
                        retval = -1;
#ifdef HAVE_SSL
                    } else if ((current_binding->ssl_key_cert == NULL) && (current_binding->required_ca != NULL)) {
                        fprintf(stderr, "RequiredCA requires ServerKey to be set in %s.\n", configfile);
                        retval = -1;
#endif
                    } else {
                ...
                if (strcmp(key, "set") == 0) {
                    if (parse_keyvaluelist(key + 4, &variables, "=") == -1) {
                        retval = counter;
                    }
...
unsigned short get_throttlespeed(char *type, t_throttle *throttle) {
    ...
    t_throttle *throt = throttle;
    unsigned long speed = 0;
    int len_type = strlen(type), len_throt;
...
#ifdef HAVE_SSL
static int mod_memadd(char *src, int src_size, char *dest, int dest_size, int dest_ofs) {
    int src_ofs = 0;

    while (src_ofs < src_size) {
        dest[dest_ofs++] ^= src[src_ofs++];
        if (dest_ofs >= dest_size) {
            dest_ofs = 0;
        }
    }
...
void fill_random_buffer(t_config *config, char *buffer, int size) {
    ...
    for (i = 0; i < (size / sizeof(time_t)); i++) {
        pos = mod_memadd((char*)&now, sizeof(time_t), buffer, size, pos);
    }
    ...
    pos = mod_memadd((char*)config, sizeof(t_config), buffer, size, pos);

    host = config->first_host;
    while (host != NULL) {
        pos = mod_memadd((char*)host, sizeof(t_host), buffer, size, pos);
        for (i = 0; i < host->hostname.size; i++) {
            pos = mod_memadd(*(host->hostname.item + i), strlen(*(host->hostname.item + i)), buffer, size, pos);
        }
        pos = mod_memadd(host->website_root, strlen(host->website_root), buffer, size, pos);
        host = host->next;
    }

    dir = config->directory;
    while (dir != NULL) {
        pos = mod_memadd((char*)dir, sizeof(t_directory), buffer, size, pos);
        pos = mod_memadd(dir->path, strlen(dir->path), buffer, size, pos);
        dir = dir->next;
    }

    binding = config->binding;
    while (binding != NULL) {
        pos = mod_memadd((char*)binding, sizeof(t_binding), buffer, size, pos);
        binding = binding->next;
    }

    cgi = config->cgi_handler;
    while (cgi != NULL) {
        pos = mod_memadd((char*)cgi, sizeof(t_cgi_handler), buffer, size, pos);
        pos = mod_memadd(cgi->handler, strlen(cgi->handler), buffer, size, pos);
        cgi = cgi->next;
    }

    mime = config->mimetype->mimename;
    while (mime != NULL) {
        pos = mod_memadd(mime->name, strlen(mime->name), buffer, size, pos);
        mime = mime->next;
    }
}
#endif
