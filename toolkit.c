...
/**
    Check if Toolkit rules are OK.
    This function is only called once from check_configuration in serverconfig.c
*/
bool toolkit_rules_oke(t_url_toolkit *url_toolkit) {
    t_toolkit_rule *rule;

    t_url_toolkit *toolkit = url_toolkit;
    while (toolkit != NULL) {
        if (toolkit->toolkit_id == NULL) {
            fputs("A ToolkitID is missing in an UrlToolkit section.", stderr);
            return false;
        }
